...
// ── Map layers ───────────────────────────────────────────────────────────────
// Carte marine : profondeurs SHOM 20 m (EMODnet en secours) décodées et ombrées
// dans le navigateur (bathy.js), terre OSM, balisage OpenSeaMap
const DEPTH_LAYER = new DepthLayer({
  pane: 'bathyPane', maxZoom: 18, maxNativeZoom: 15,
  attribution: '© SHOM (MNT TANDEM 20 m) · © EMODnet',
});
...
async function showDepthAt(latlng) {
  const popup = L.popup({ className: 'depth-popup', closeButton: false, autoPan: false })
    .setLatLng(latlng).setContent('<div class="dp-v">…</div>');
  const show = z => {
  ...
  const local = DEPTH_LAYER.depthAt(latlng);
  if (local !== undefined) { popup.openOn(map); show(local); return; }
  ...
  try {
    ...
    show(r.features?.[0]?.properties?.Depth);
  } catch {
...
function _wikiHome() {
  return `<div class="wiki-home">
    ...
    <div class="wiki-cats">
      ...
      <div class="wiki-cat wiki-cat-port" onclick="wikiNav('port')">
        <div class="wiki-cat-ico">⚓</div>
  ...
  return `
    <div style="padding:16px">
      ...
      <div style="text-align:center;margin:20px 0 8px">
        ...
        <div style="font-size:22px;font-weight:800;color:var(--text);margin:4px 0">Accès Port</div>
      ...
      <div style="background:var(--surf);border-radius:var(--r);padding:14px;margin-bottom:12px">
        <div style="font-size:11px;font-weight:700;color:var(--text2);letter-spacing:.08em;margin-bottom:8px">INFORMATIONS</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div style="background:var(--surf2);border-radius:10px;padding:12px">
            <div style="font-size:10px;font-weight:700;color:var(--text2);letter-spacing:.06em">PONTON</div>
            <div style="font-size:16px;font-weight:800;color:var(--sea);margin-top:2px">VST</div>
          </div>
          <div style="background:var(--surf2);border-radius:10px;padding:12px">
      ...
      <div style="background:var(--surf);border-radius:var(--r);padding:14px">
        <div style="font-size:11px;font-weight:700;color:var(--text2);letter-spacing:.08em;margin-bottom:8px">POINTS D'ACCÈS</div>
